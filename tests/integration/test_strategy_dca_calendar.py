"""Phase 3 Wave 1 Task 2.9 — B1 Calendar DCA integration test."""
...
def test_dca_calendar_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "dca_calendar" in registry
        assert registry["dca_calendar"] is CalendarDcaStrategy

        deploy = asyncio.run(strategies_command.deploy_strategy(
            strategy_type="dca_calendar",
            symbol="BTCUSDT",
            capital_usd=1000,
            params={"contribution_usd": "25", "interval_seconds": 604800},
            rationale="B1 Calendar DCA integration test",
        ))
        ...
        async def _tick_with_now(row, cls):
            ...
            instance.tick(ctx, snapshot={"now": now})
        ...
        monkeypatch.setattr(worker, "_tick_one_strategy", _tick_with_now)
        try:
        ...
        assert state["total_contributed_usd"] == "25"
