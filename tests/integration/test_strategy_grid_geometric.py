"""Phase 3 Wave 1 Task 2.3 — A3 Geometric Grid integration test."""
...
def test_grid_geometric_deploys_and_ticks(env_for_postgres, monkeypatch):
    from trading_sandwich.mcp.tools import strategies_command
    from trading_sandwich.strategies import worker
    from trading_sandwich.strategies.grid.geometric import (
        GeometricGridStrategy,
    )
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "grid_geometric" in registry, (
            "grid_geometric must be in worker._default_registry()"
        )
        assert registry["grid_geometric"] is GeometricGridStrategy

        deploy = asyncio.run(strategies_command.deploy_strategy(
            strategy_type="grid_geometric",
            symbol="SOLUSDT",
            capital_usd=60,
            params={"low": "100", "pct_spacing": "0.02", "levels": 6},
            rationale="A3 Geometric Grid integration test",
        ))
        assert deploy["status"] == "ok", deploy
        sid = deploy["strategy_id"]

        rows = _query(url,
            "SELECT strategy_type, status FROM strategies WHERE id = :i",
            {"i": sid})
        assert rows == [("grid_geometric", "active")]
        ...
        assert submitted.count(True) == 3  # 100, 102, 104.04 <= 105
