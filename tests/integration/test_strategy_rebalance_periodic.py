"""Phase 3 Wave 1 Task 2.14 — C1 Periodic Rebalancing integration test."""
...
def test_rebalance_periodic_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "rebalance_periodic" in registry
        assert registry["rebalance_periodic"] is PeriodicRebalanceStrategy

        deploy = asyncio.run(strategies_command.deploy_strategy(
            strategy_type="rebalance_periodic",
            symbol="BTCUSDT",
            capital_usd=1000,
            params={"target_fraction": "0.5", "interval_seconds": 2592000},
            rationale="C1 Periodic Rebalancing integration test",
        ))
        ...
        # 0.5*1000 = 500 USD at 50000 → 0.01 units
