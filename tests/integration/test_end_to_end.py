...
def _seed_candles_for_pullback(async_url: str, n: int = 260) -> datetime:
    """Seed n 1m candles — 200+ warmup bars then a pullback pattern near the end
    so the full Phase 1 indicator stack can compute and the detector chain runs.
    """
    ...
    async def _run() -> None:
        ...
        try:
            async with engine.begin() as conn:
                for i in range(n):
                    close = 100 + i * 0.5
                    # Last 3 bars: pullback then bounce (same as Phase 0's Task 26 pattern)
                    if i == n - 3:
                        close = 100 + (n - 5) * 0.5
                    if i == n - 2:
                        close = 100 + (n - 6) * 0.5
                    if i == n - 1:
                        close = 100 + n * 0.5 + 1.5
                    v = 10 + (i % 7) * 0.5   # non-constant volume so z-score defined
                    ...
                    await conn.execute(text(
                        ...
                        "VALUES (:s,:t,:ot,:ct,:o,:h,:l,:c,:v)"
                    ), {"s": "BTCUSDT", "t": "1m", "ot": ot, "ct": ct,
                        "o": close - 0.1, "h": close + 0.3, "l": close - 0.3, "c": close, "v": v})
        finally:
...
def test_end_to_end_candle_to_features_and_signal(env_for_postgres, env_for_redis):
    ...
    with (
        ...
        # signals row depending on the exact seeded pattern + regime; the
        # signals-worker integration test asserts detector behavior on a
        # pattern engineered to fire.
