"""Phase 3 Wave 1 Task 2.20 — D3 Volatility Breakout integration test."""
...
def test_trend_volatility_breakout_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "trend_volatility_breakout" in registry
        assert registry["trend_volatility_breakout"] is VolatilityBreakoutStrategy

        deploy = asyncio.run(strategies_command.deploy_strategy(
            strategy_type="trend_volatility_breakout",
            symbol="BTCUSDT",
            capital_usd=200,
            params={"position_usd": "100", "k_atr": "1.0"},
            rationale="D3 Volatility Breakout integration test",
        ))
        ...
        async def _tick_with_breakout(row, cls):
        ...
        monkeypatch.setattr(worker, "_tick_one_strategy", _tick_with_breakout)
        try:
        ...
        assert Decimal(state["position_units"]) == Decimal("100") / Decimal("51500")
