"""Phase 3 Wave 1 Task 2.12 — B4 Indicator-Triggered DCA integration test."""
...
def test_dca_indicator_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "dca_indicator" in registry
        assert registry["dca_indicator"] is IndicatorTriggeredDcaStrategy
        ...
        deploy = asyncio.run(strategies_command.deploy_strategy(
            ...
            rationale="B4 Indicator-Triggered DCA integration test",
        ))
        ...
        async def _tick_with_oversold(row, cls):
            ...
            instance.tick(ctx, snapshot={"now": now, "rsi": Decimal("25")})
        ...
        monkeypatch.setattr(worker, "_tick_one_strategy", _tick_with_oversold)
        try:
        ...
        assert Decimal(state["total_contributed_usd"]) == Decimal("15")
