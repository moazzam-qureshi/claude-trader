"""Phase 3 Wave 1 Task 2.22 — D5 Multi-TF Alignment integration test."""
...
def test_trend_multi_tf_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "trend_multi_tf_alignment" in registry
        assert registry["trend_multi_tf_alignment"] is MultiTfAlignmentStrategy
        ...
        deploy = asyncio.run(strategies_command.deploy_strategy(
            ...
            rationale="D5 Multi-TF Alignment integration test",
        ))
        ...
        async def _tick_all_bullish(row, cls):
            ...
            instance.tick(ctx, snapshot={
                ...
                "bullish_1d": True, "bullish_4h": True, "bullish_1h": True,
            })
        ...
        monkeypatch.setattr(worker, "_tick_one_strategy", _tick_all_bullish)
        try:
        ...
        assert Decimal(state["position_units"]) == Decimal("100") / Decimal("51000")
