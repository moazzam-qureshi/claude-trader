"""Phase 3 Wave 1 Task 2.4 — A4 Reverse Grid integration test."""
...
def test_grid_reverse_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "grid_reverse" in registry
        assert registry["grid_reverse"] is ReverseGridStrategy
        ...
        deploy = asyncio.run(strategies_command.deploy_strategy(
            ...
            rationale="A4 Reverse Grid integration test",
        ))
        ...
        rows = _query(url,
            ...
            {"i": sid})
        assert rows == [("grid_reverse", "active")]
        ...
        # mid=65000 → rungs at 65k, 67.5k, 70k are >= mid → 3 sells submitted.
        submitted = [lv["submitted"] for lv in state["levels"]]
        assert submitted.count(True) == 3
        assert all(lv["side"] == "sell" for lv in state["levels"])
