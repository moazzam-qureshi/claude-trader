"""Phase 3 Wave 1 Task 2.10 — B2 Value Averaging integration test."""
...
def test_dca_value_averaging_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "dca_value_averaging" in registry
        assert registry["dca_value_averaging"] is ValueAveragingStrategy

        deploy = asyncio.run(strategies_command.deploy_strategy(
            strategy_type="dca_value_averaging",
            symbol="BTCUSDT",
            capital_usd=5000,
            params={"base_growth_usd": "100", "interval_seconds": 604800},
            rationale="B2 Value Averaging integration test",
        ))
        ...
        async def _tick_with_snapshot(row, cls):
            ...
            instance.tick(ctx, snapshot={
                "now": now, "mid_price": Decimal("50000"),
            })
        ...
        assert Decimal(state["total_contributed_usd"]) == Decimal("100")
        # 100 USD at 50000 → 0.002 units
        assert Decimal(state["position_units"]) == Decimal("0.002")
