"""Phase 3 Wave 1 Task 2.23 — E3 BTC Dominance Rotation integration test."""
...
def test_rotation_btc_dominance_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "rotation_btc_dominance" in registry
        assert registry["rotation_btc_dominance"] is BtcDominanceRotationStrategy
        ...
        deploy = asyncio.run(strategies_command.deploy_strategy(
            ...
            rationale="E3 BTC Dominance Rotation integration test",
        ))
        ...
        # btc + rising → heavy → 0.8*1000 = 800 at 50000 → 0.016 units
        assert Decimal(state["position_units"]) == Decimal("0.016")
