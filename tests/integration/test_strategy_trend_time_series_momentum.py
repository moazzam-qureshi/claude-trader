"""Phase 3 Wave 1 Task 2.21 — D4 Time-Series Momentum integration test."""
...
def test_trend_tsm_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "trend_time_series_momentum" in registry
        assert registry["trend_time_series_momentum"] is TimeSeriesMomentumStrategy
        ...
        deploy = asyncio.run(strategies_command.deploy_strategy(
            ...
            rationale="D4 Time-Series Momentum integration test",
        ))
        ...
        async def _tick_above_ma(row, cls):
            ...
            instance.tick(ctx, snapshot={
                "mid_price": Decimal("51000"), "ma_n": Decimal("50000"),
            })
        ...
        monkeypatch.setattr(worker, "_tick_one_strategy", _tick_above_ma)
        try:
