"""Phase 3 Wave 1 Task 2.5 — A5 RSI Mean Reversion integration test."""
...
def test_rsi_mean_reversion_deploys_and_ticks(env_for_postgres, monkeypatch):
    ...
    with PostgresContainer("pgvector/pgvector:pg16", driver="asyncpg") as pg:
        ...
        assert "rsi_mean_reversion" in registry
        assert registry["rsi_mean_reversion"] is RsiMeanReversionStrategy
        ...
        deploy = asyncio.run(strategies_command.deploy_strategy(
            ...
            rationale="A5 RSI Mean Reversion integration test",
        ))
        ...
        rows = _query(url,
            ...
            {"i": sid})
        assert rows == [("rsi_mean_reversion", "active")]

        original = worker._tick_one_strategy

        async def _tick_with_oversold_rsi(row, cls):
        ...
        monkeypatch.setattr(worker, "_tick_one_strategy", _tick_with_oversold_rsi)
        try:
        ...
        assert state["last_signal_kind"] == "oversold"
